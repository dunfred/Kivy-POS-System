...
class OperatorWindow(BoxLayout):
    ...
    def update_purchases(self):
        ...
        if pcode == '1234' or '2345':
            ...
            #Add label widgets to details boxlayout
            ...
            pqty = str(1)
            self.total += pprice
            purchase_total = "`\n\nTotal\t\t\t\t\t\t\t\tGHs" + str(self.total)
            self.ids.cur_product.text = pname
            self.ids.cur_price.text = "GHs "+ str(pprice)
            ...
            if ptarget >= 0:
                ...
                self.qty.append(1)
                nu_preview = '\n'.join([prev_text, pname+"\t\tx"+pqty+"\t\tGHs"+str(pprice), purchase_total])
